...
// Load pipe images before using them in createPipes()
...
function handleKeyDown(event) {
    if (inputLocked) return; // Prevent early jump
...
function handleMouseDown(event) {
    ...
    if (currentState === GAME_STATE.MENU) {
        ...
        if (
            clickX >= playButton.x && clickX <= playButton.x + playButton.width &&
...
function startGame() {
    ...
    inputLocked = true; // Lock input temporarily
    setTimeout(() => {
        inputLocked = false; // Unlock input after 1 second
    }, 1000);

    // Delay gravity effect
    setTimeout(() => {
